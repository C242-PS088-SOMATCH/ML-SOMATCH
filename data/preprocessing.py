...
flags.DEFINE_string('train_label', 'data/label/train_no_dup.json',
                           'Training label file')
flags.DEFINE_string('test_label', 'data/label/test_no_dup.json',
                           'Testing label file')
flags.DEFINE_string('valid_label','data/label/valid_no_dup.json',
                           'Validation label file')
flags.DEFINE_string('output_directory', 'data/tf_records/',
                           'Output data directory')
flags.DEFINE_string('image_dir', 'data/images/',
                           'Directory of image patches')
flags.DEFINE_string('word_dict_file', 'data/final_word_dict.txt',
                           'File containing the word dictionary.')
flags.DEFINE_integer('train_shards', 10,
                            'Number of shards in training TFRecord files.')
flags.DEFINE_integer('test_shards', 5,
                            'Number of shards in test TFRecord files.')
flags.DEFINE_integer('valid_shards', 5,
...
def _int64_feature(value):
    ...
    if not isinstance(value, list):
        value = [value]  # Pastikan value adalah list
...
def _bytes_feature_list(values):
    ...
    # Mengonversi setiap kata dalam list menjadi bytes secara terpisah
...
class ImageCoder(object):
    ...
    def __init__(self):
        # TensorFlow 2.x tidak memerlukan tf.Session, karena eager execution aktif secara default.
        # Inisialisasi fungsi untuk mengkonversi PNG ke JPEG
        self._png_to_jpeg = tf.function(self._convert_png_to_jpeg)
        
        # Inisialisasi fungsi untuk mendekode data JPEG
        self._decode_jpeg = tf.function(self._decode_jpeg_data)

    def _convert_png_to_jpeg(self, image_data):
        """Convert PNG to JPEG."""
        image = tf.image.decode_png(image_data, channels=3)
        return tf.image.encode_jpeg(image, format='rgb', quality=100)

    def _decode_jpeg_data(self, image_data):
    ...
    def png_to_jpeg(self, image_data):
        ...
        # Pada TensorFlow 2.x, Anda tidak perlu lagi menggunakan sesi eksplisit.
        return self._png_to_jpeg(image_data)

    def decode_jpeg(self, image_data):
        """Decode JPEG image data to an RGB image."""
        image = self._decode_jpeg(image_data)
        assert len(image.shape) == 3  # Memastikan gambar memiliki 3 dimensi (tinggi, lebar, kanal)
        assert image.shape[2] == 3  # Memastikan gambar memiliki 3 kanal warna (RGB)
        return image
...
def _to_sequence_example(set_info, decoder, vocab):
  ...
  for image_info in set_info['items']:
    ...
    caption_data.append(caption_bytes)
...
def _process_image_files_batch(coder, thread_index, ranges, name,
  ...
  for s in range(num_shards_per_batch):
    ...
    # Change to tf.io.TFRecordWriter for TensorFlow 2.x
...
def _process_image_files(name, all_sets, vocab, num_shards):
    ...
    spacing = np.linspace(0, len(all_sets), FLAGS.num_threads + 1).astype(np.int32)
    ranges = []
    for i in range(len(spacing) - 1):  # Ganti xrange dengan range
    ...
    threads = []
    for thread_index in range(len(ranges)):  # Ganti xrange dengan range
...
def _find_image_files(labels_file, name):
  ...
  print(f'Found {len(all_sets)} fashion sets.')
  return all_sets
